    ...
    return (
        <>
            ...
            <div
                ...
                onClick={() => setIsOpen((state) => !state)}
            />
            <div className={`fixed w-full max-w-80 h-screen bg-white z-50 top-0 transition-all duration-300 ease-out ${isOpen ? 'right-0' : '-right-80'}`}>
                <FilterOption setIsOpen={setIsOpen} />
            </div>
        </>
    );
};
